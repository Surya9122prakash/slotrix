...
export const normalizeDate = (d, timezone) => moment.utc(d).tz(timezone);
export const getWorkingHoursRange = (enabledTimeInterval) => {
...
export const generateTimeSlots = (startOfDay, slotInterval) => {
    const slots = [];
...
export const checkIsSlotEnabled = (slot, enabledTimeSlots, disabledTimeSlots, enabledTimeInterval, disableTimeInterval) => {
    ...
    const isWithinInterval = (intervals, invert = false) => {
        if (!intervals?.length)
...
export const getDayEvents = (safeEvents, targetDate, timezone) => {
    const result = [];
...
export const calculateLayoutEvents = (dayEvents, targetDate, slotInterval) => {
    const sorted = [...dayEvents].sort((a, b) => a.start.valueOf() - b.start.valueOf());
    const columns = [];
    sorted.forEach((event) => {
        let placed = false;
        for (const column of columns) {
            const last = column[column.length - 1];
            if (event.start.isSameOrAfter(last.end)) {
                column.push(event);
                placed = true;
                break;
            }
    ...
    return sorted.map((event) => {
        ...
        const top = (startMoment.diff(targetDate.clone().startOf("day"), "minutes") /
            slotInterval) *
            SLOT_HEIGHT;
        const height = (endMoment.diff(startMoment, "minutes") / slotInterval) * SLOT_HEIGHT;
        return {
            ...event,
            columnIndex,
            columnCount,
...
export const detectConflicts = (events, timezone) => {
    const conflicts = [];
    const sorted = [...events].sort((a, b) => moment(a.start).tz(timezone).valueOf() - moment(b.start).tz(timezone).valueOf());
